...
if (!class_exists("Bread")) {
    ...
    // phpcs:enable PSR1.Classes.ClassDeclaration.MissingNamespace
    {
        ...
        public $formats_used = '';
        public $formats_by_key = array();
        public $formats_spanish = '';
        public $formats_all = '';
        public $translate = array();
        public $services = '';
        public $meeting_fields = array (
            'id_bigint',
            'service_body_bigint',
            'weekday_tinyint',
            'start_time',
        ...
        public $section_shortcodes;
        public $service_meeting_result = null;
        const SETTINGS = 'bmlt_meeting_list_settings';
        const OPTIONS_NAME = 'bmlt_meeting_list_options';
        public $optionsName = Bread::OPTIONS_NAME;
        public $options = array();
        public $outside_meeting_result = array();
        public $allSettings = array();
        public $maxSetting = 1;
        public $loaded_setting = 1;
        public $authors_safe = array();
        ...
        public function getCurrentMeetingListHolder()
        {
            $ret = array();
        ...
        public function ml_default_editor()
        {
        ...
        public function force_mce_refresh($ver)
        {
            global $my_admin_page;
            $screen = get_current_screen();
            if ($screen->id == $my_admin_page) {
        ...
        public function my_sideload_image()
        {
            global $my_admin_page;
            $screen = get_current_screen();
        ...
        public function tiny_tweaks($initArray)
        {
            global $my_admin_page;
            $screen = get_current_screen();
            if ($screen->id == $my_admin_page) {
        ...
        public function clear_admin_message2()
        {
        ...
        */
        ...
        public function get_bmlt_server_lang()
        {
        ...
        public function testRootServer($override_root_server = null)
        {
        ...
        // meeting list.
        public function getFormatsForSelect($all = false)
        {
            if ($all) {
        ...
        public function bmlt_meeting_list($atts = null, $content = null)
        {
            ...
            } elseif ($this->options['page_fold'] == 'full' && $this->options['booklet_pages']) {
                $this->mpdf->Output($FilePath, 'F');
                $mpdfOptions = [
                    'mode' => $mode,
                    'tempDir' => get_temp_dir(),
        ...
        public function orderByWeekdayStart(&$result_meetings)
        {
        ...
        public function writeMeetings($result_meetings, $template, $lang, $include_asm)
        {
        ...
        //               1  -  only meetings with asm format
        //              -1  -  only meetings without asm format
        public function getHeaderMeetings(&$result_meetings, $lang, $include_asm)
        {
            $levels = $this->getHeaderLevels();
            foreach ($result_meetings as &$value) {
                $enFormats = explode(",", $value['formats']);
                if ($include_asm < 0 && in_array($this->options['asm_format_key'], $enFormats)) {
                    continue;
                }
                if ($include_asm > 0 && !in_array($this->options['asm_format_key'], $enFormats)) {
                    continue;
        ...
        public function writeBreak($mpdf)
        {
            if ($this->options['page_fold'] === 'half' || $this->options['page_fold'] === 'full') {
        ...
        public function addColumnSeparators($oh)
        {
            if ($this->options['column_line'] == 1) {
        ...
        public function getHeaderLevels()
        {
        ...
        public function getHeaderItem($value, $name)
        {
        ...
        public function upgradeHeaderData()
        {
        ...
        public function get_area_name($meeting_value)
        {
            foreach ($this->unique_areas as $unique_area) {
                $area_data = explode(',', $unique_area);
                $area_id = $area_data[1];
        ...
        public function analyseTemplate($template)
        {
        ...
        public function enhance_meeting(&$meeting_value, $lang)
        {
        ...
        public function write_single_meeting($meeting_value, $template, $analysedTemplate, $area_name)
        {
            $data = $template;
            $namedValues = array();
        ...
        public function get_booklet_pages($np, $backcover = true)
        {
            $lastpage = $np;
            $np = 4*ceil($np/4);
            $pp = array();
        ...
        public function write_last_page()
        {
        ...
        public function write_custom_section()
        {
            $this->mpdf->SetHTMLHeader();
        ...
        public function locale_month_replacement($data, $case, $sym)
        {
        ...
        public function standard_shortcode_replacement(&$data, $page)
        {
            $search_strings = array();
            $replacements = array();
        ...
        public function writeHTMLwithServiceMeetings($data, $page)
        {
        ...
        public function replace_format_shortcodes(&$data, $page_name)
        {
        ...
        public function shortcode_formats($shortcode, $detailed, $formats, $page, &$str)
        {
        ...
        public function write_formats($formats, $page)
        {
            if ($formats == null) {
                return '';
            }
        ...
        public function asm_required($data)
        {
            return strpos($data, '[service_meetings]') || strpos($data, '[additional_meetings]');
        }
        public function write_detailed_formats($formats, $page)
        {
            if ($formats == null) {
                return '';
            }
        ...
        public function get_field($obj, $field)
        {
            $value = '';
            if (isset($obj[$field])) {
        ...
        public function write_service_meetings($font_size, $line_height)
        {
        ...
        */
        ...
        public function bmltrootserverurl_meta_box()
        {
            global $connect;
            ?>
        ...
        */
        ...
        public function getSingleLanguage($lang)
        {
        ...
        public function fillUnsetOption($option, $default)
        {
            if (!isset($this->options[$option]) || strlen(trim($this->options[$option])) == 0) {
                $this->options[$option] = $default;
            }
        }
        public function fillUnsetStringOption($option, $default)
        {
         ...
         */
        public function count_transient_cache()
        {
            global $wpdb, $_wp_using_ext_object_cache;
            wp_cache_flush();
        ...
        public function pwsix_process_settings_admin()
        {
        ...
        public function pwsix_process_rename_settings()
        {
            if (isset($_POST['bmltmeetinglistsave']) && $_POST['bmltmeetinglistsave'] == 'Save Changes') {
                return;
            }
         ...
         */
        public function pwsix_process_settings_export()
        {
            if (isset($_POST['bmltmeetinglistsave']) && $_POST['bmltmeetinglistsave'] == 'Save Changes') {
                return;
            }
        ...
        public function current_user_can_modify()
        {
         ...
         */
        public function pwsix_process_settings_import()
        {
            if (isset($_POST['bmltmeetinglistsave']) && $_POST['bmltmeetinglistsave'] == 'Save Changes') {
                return;
            }
         ...
         */
        public function pwsix_process_default_settings()
        {
            if (! current_user_can('manage_bread') ||
