...
<div id="poststuff">
    <div id="postbox-container" class="postbox-container">
        <div id="normal-sortables" class="meta-box-sortables ui-sortable">
            <div id="exportdiv" class="postbox">
                <h3 class="hndle"><?php esc_html_e('Configuration Manager', 'bread') ?></h3>
                <div class="inside">
                    <p><?php esc_html_e("Bread can support multiple meeting lists.  Each meeting list has an integer ID and a text description that help the user to identify
                        ...
                        that generates the meeting list (eg, ?current-meeting-list=2 generates the meeting list with ID 2).", 'bread') ?></p>
                    <h4><?php esc_html_e('Current Meeting List', 'bread') ?></h4>
                    <form method="post">
                        <p><?php esc_html_e('Meeting List ID: ', 'bread') ?><?php echo esc_html($bread->getRequestedSetting()) ?>
                        ...
                        <br><?php esc_html_e('Configuration Name: ', 'bread') ?><input type="text" name="setting_descr" value="<?php echo esc_attr($bread->getSettingName($bread->getRequestedSetting())) ?>" />
                        <?php submit_button(__('Save Configuration Name', 'bread'), 'button-primary', 'submit_settings_name', false); ?>
                    </form>
                    </p>
                    <?php if (count($bread->getSettingNames()) > 1) { ?>
                        <h4><?php esc_html_e('Configuration Selection', 'bread') ?></h4>
                        <form method="post">
                            <label for="current_setting"><?php esc_html_e('Select Configuration: ', 'bread') ?></label>
                            ...
                            <?php submit_button(__('Load Configuration', 'bread'), 'button-primary', 'submit_change_settings', false); ?>
                        </form>
                    <?php } ?>
                    <hr />
                    <form method="post">
        ...
        <div id="normal-sortables" class="meta-box-sortables ui-sortable">
            <div id="exportdiv" class="postbox">
                <h3 class="hndle"><?php esc_html_e('Export Configuration', 'bread') ?></h3>
                <div class="inside">
                    ...
                    <form method="post">
                        ...
                        <p>
                            ...
                            <?php submit_button(__('Export', 'bread'), 'button-primary', 'submit', false); ?>
                        </p>
                    </form>
                </div>
            </div>
            <div style="margin-bottom: 0px;" id="exportdiv" class="postbox">
                <h3 class="hndle"><?php esc_html_e('Import Configuration', 'bread'); ?></h3>
                <div class="inside">
                    ...
                    <form id="form_import_file" method="post" enctype="multipart/form-data">
                        ...
                        <p>
                            ...
                            <?php submit_button(__('Import', 'bread'), 'button-primary', 'submit_import_file', false, array('id' => 'submit_import_file')); ?>
                        </p>
                        <div id="basicModal">
                            <p style="color:#f00;"><?php esc_html_e('Your current meeting list settings will be replaced and lost forever.', 'bread'); ?></p>
                            <p><?php esc_html_e('Consider backing up your settings by using the Export function.', 'bread') ?></p>
                        </div>
                        <div id="nofileModal" title="File Missing">
                            <div style="color:#f00;"><?php esc_html_e('Please Choose a File.', 'bread'); ?></div>
                        </div>
                    </form>
                </div>
            </div>
    ...
    <br class="clear">
    <p style="display: inline; margin-top:.5em;margin-bottom:1.0em;margin-left:.2em;"><a target="_blank" class="button-primary" href="<?php echo esc_url(home_url()."/?current-meeting-list=".$bread->getRequestedSetting()); ?>"><?php esc_html_e('Generate Meeting List', 'bread'); ?></a></p>
</div>
<?php }