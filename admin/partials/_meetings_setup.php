        ...
        <div id="normal-sortables" class="meta-box-sortables ui-sortable">
            ...
            <div class="postbox">
                ...
                <div class="inside">
                    ...
                    <?php if ($this_connected) { ?>
                        <option value="">Not Used</option>
                        <option value="@Virtual@" <?php echo Bread::getOption('asm_format_key')=='@Virtual@' ? 'selected' : '' ?>>Virtual Meetings</option>
                        <option value="@F2F@" <?php echo Bread::getOption('asm_format_key')=='@F2F@' ? 'selected' : '' ?>>Face-to-Face Meetings</option>
                        <?php
                        if ($this_connected) {
                        ...
                        for ($count = 0; $count < $countmax; $count++) {
                            if ($used_formats[$count]['key_string'] == Bread::getOption('asm_format_key')) { ?>
                            ...
                            <option selected="selected" value="<?php echo Bread::getOption('asm_format_key'); ?>"><?php echo 'ASM'; ?></option>
                    <?php } ?>
                    </select>
                    </p><p>
                    <label for="asm_sort_order">Select sort order for the additional list</label>
                    <select id="asm_sort_order" name="asm_sort_order">
                        <option value="meeting_name" <?php echo Bread::getOption('asm_sort_order')=='meeting_name' ? 'selected' : ''; ?> >By Name</option>
                        <option value="weekday_tinyint,start_time" <?php echo Bread::getOption('asm_sort_order')=='weekday_tinyint,start_time' ? 'selected' : ''; ?> >By Day and Time</option>
                        <option value="same"<?php echo Bread::getOption('asm_sort_order')=='same' ? 'selected' : ''; ?> >Same as main list</option>
                     </select>
                     </p><p>
                    <label for="asm_language">Select language for the additional list</label>
                    <select id="asm_language" name="asm_language">
                        <?php
                        if (Bread::getOption('asm_language') == '') {
                            echo "<option value=\"\" selected=\"selected\">Same as main list</option>";
                        } else {
                            echo "<option value=\"\">Same as main list</option>";
                        }
                        foreach (Bread::getTranslateTable() as $key => $value) {
                            if (Bread::getOption('asm_language') == $key) {
                                echo "<option value=\"$key\" selected=\"selected\">".$value['LANG_NAME']."</option>";
                            } else {
                                echo "<option value=\"$key\">".$value['LANG_NAME']."</option>";
                            }
                    ...
                    <input type="text" id="asm_custom_query" name="asm_custom_query" size="100" value="<?php echo esc_html(Bread::getOption('asm_custom_query'))?>" />
                    </p><p>
                    ...
                    <?php if ($logged_in == 'OK') { ?>
                        <div id="includeemaildiv" class="inside">
                            ...
                            <input name="include_meeting_email" value="0" type="hidden"><p><input type="checkbox" name="include_meeting_email" value="1" <?php echo (Bread::getOption('include_meeting_email') == '1' ? 'checked' : '') ?>>Enable</p>
                        </div>
                    <?php } ?>
                    <input name="include_asm" value="0" type="hidden">
                    <p><input type="checkbox" name="include_asm" value="1" <?php echo (Bread::getOption('include_asm') == '1' ? 'checked' : '') ?>>Include meetings with this format in the main list</p>
